...
import { FipeService } from '../../services/api/fipeService';
import { FipeBrand, FipeModel, FipeYear } from '../../@types/fipe.types';
...
const VehicleForm: React.FC<VehicleFormProps> = ({
  ...
  // Limpar dados FIPE ao fechar modal
  useEffect(() => {
    if (!visible) {
      setFipeModels([]);
      setFipeYears([]);
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: 'var(--space-xxl)', background: 'var(--gray-1)' }}>
        {!isEditing && (
          <Alert
            message={
              <Space>
                <DatabaseOutlined />
                <Text strong>Cadastro Inteligente</Text>
              </Space>
            }
            description={
              fipeApiAvailable
                ? "Dados oficiais da FIPE! Selecione a marca e os modelos serão carregados automaticamente."
                : "Base FIPE temporariamente indisponível. Use entrada manual."
            }
            type={fipeApiAvailable ? "info" : "warning"}
            showIcon={false}
            style={{
              marginBottom: 'var(--space-lg)',
              border: `1px solid ${fipeApiAvailable ? 'var(--primary-color)' : 'var(--warning-color)'}`,
              borderRadius: 'var(--border-radius-md)',
              background: fipeApiAvailable ? '#f0f5ff' : '#fff7e6'
            }}
            action={
              !fipeApiAvailable && (
                <Button
