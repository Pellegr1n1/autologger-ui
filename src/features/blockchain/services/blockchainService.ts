...
export class BlockchainService {
  ...
  // Reenviar serviço falhado
  static async resendFailedService(serviceId: string): Promise<ServiceSubmissionResult> {
    const response = await apiBase.api.post<ServiceSubmissionResult>(
      `${this.BASE_PATH}/services/${serviceId}/resend`
    );
    return response.data;
  }
  ...
  // Corrigir datas incorretas nos serviços
  static async fixIncorrectDates(): Promise<{ success: boolean; message: string; correctedCount: number; totalProcessed: number }> {
    const response = await apiBase.api.post<{ success: boolean; message: string; correctedCount: number; totalProcessed: number }>(
      `${this.BASE_PATH}/services/fix-dates`
    );
    return response.data;
  }
